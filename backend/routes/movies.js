...
Router.get("/giveUp", (req, res) => {
  ...
  mysqlConnection.query(sql, [i, query.userID], (err__, results__, fields__) => {
    ...
    sql = "WITH guessed_movie AS ( \
    SELECT id.mlID \
    FROM dailyMovies dm, tmdbPopularMovies tm, idLinks id \
    WHERE dm.selectID = tm.selectID \
      AND tm.tmdbID = id.tmdbID \
      AND dm.challengeDate = CURDATE() \
    ), \
    daily_movie_year AS ( \
      SELECT ml.releaseYear \
      FROM dailyMovies dm \
    ...
    mysqlConnection.query(sql, [], (err___, results___, fields___) => {
      if (err___) {
        console.error('Error executing query:', err___);
        return;
      }
